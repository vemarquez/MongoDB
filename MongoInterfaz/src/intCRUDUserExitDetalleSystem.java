...
public class intCRUDUserExitDetalleSystem {
	...
	public static String intCreateUserExitDetalleSystem( Connection con, DB db, String sTabla ) {		
		...
		try {
			...
			while (resUserExitDetalleSystem.next()) {				
				...
				sError = sCreateMDUserExitDetalleSystem( db,  sTabla, lId, lIdTarea, sSalida );				
				if( ! sError.equals("1"))
					return sError;				
			}
		}
	...
	private static String sCreateMDUserExitDetalleSystem(DB db, String sTabla, long lId, long lIdTarea, String sSalida ) {
		String sError;
		try {
